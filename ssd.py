...
class BBoxUtility(object):
    ...
    def iou(self, box: 'shape (4,)')->'shape (num_priors)':
    ...
    def encode_box(self, box: 'shape (4,)', return_iou=True)->'shape (num_priors, 4 + int(return_iou))':
        """encode bbox
        """
        iou                              = self.iou(box)
        encoded_box                      = np.zeros((self.num_priors, 4 + return_iou))
        assign_mask                      = iou > self.overlap_threshold
        assign_mask[iou.argmax()]        = True if not assign_mask.any() else assign_mask[iou.argmax()]
        encoded_box[:, -1][assign_mask]  = iou[assign_mask] if return_iou else encoded_box[:, -1][assign_mask]
    ...
    def assign_boxes(self, boxes: 'shape (num_boxes, 4 + num_classes)')->'shape (num_boxes, 4 + num_classes + 8)':
        """assign boxes to priors
        """
